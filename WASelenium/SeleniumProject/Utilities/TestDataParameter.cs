            ...
            if (parentDictionary == null)
            {
                ...
                if (parentDictionary.Count == 0)
                {
                    foreach (string item in strarr)
                    {
			            ...
			            {
				            if (ws.Cells[row, 1].Value != null && ws.Cells[row, 1].Value.ToString().Trim() == testCaseName)
				            {
					            startRow = row;
				            ...
				            {
					            if (ws.Cells[row, column].Value != null && ws.Cells[row, column].Value.ToString().Trim() == testCaseName)
					            {
						            endRow = row;
						            endColumn = column;
						            break;
